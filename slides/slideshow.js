...
var host_str = "http://www.kamusiproject.org/"
Slideshow = new Class({
...
//-------------------------------------------------------
	...
	start: function() {
		...
		this.loader.src = host_str+this.props.hu + this.props.images[this.curr[0]].trim();
		...
		if (this.props.navigation.test(/arrows\+|thumbnails/)) {
			for (i = 0; i < this.props.images.length; i++) {
				...
				if (this.props.navigation.test(/thumbnails/)) {
					src = host_str+this.props.hu + this.props.images[i].trim().replace(this.props.thumbnailre[0], this.props.thumbnailre[1]);
